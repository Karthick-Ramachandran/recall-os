...
description:
  "Record durable context from MCP servers and design or project tools into repository memory as
  proposed. Use when working with an MCP server like Figma, Linear, Jira, or Sentry, or after
  pulling design, ticket, or error context, to persist it for future sessions."
---
...
Persist the durable parts of MCP-derived external context so future sessions remember them instead
of re-deriving them.
...
1. Identify the MCP server and the durable facts worth remembering (design tokens, component
   mappings, ticket acceptance criteria, recurring error signatures).
2. If `docs/ai/mcp/<server>.md` does not exist, create it with `persist mcp add <server>`.
3. Record the durable context in the Captured Context section as proposed memory, with enough detail
   to reuse.
