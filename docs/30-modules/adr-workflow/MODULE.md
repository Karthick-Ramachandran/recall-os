...
`persist adr create <title>` creates proposed ADR memory before a decision is accepted.

`persist adr accept <name>` promotes a proposed ADR to accepted repository memory.

## Current Decision

`persist adr create` creates concise proposed ADR drafts.

`persist adr accept` (ADR-0006) closes the propose-to-accept loop: a proposal under
