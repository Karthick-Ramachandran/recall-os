# Definition Of Done

## Done Means Evidence Exists

A Recall OS task is not done because an agent says it is done.

Done requires:

- Requirements are linked to PRD, feature docs, or accepted change requests.
- Module requests have feature delivery docs before task execution.
- Architecture impact is considered.
- ADR impact is considered.
- Engineering standards are followed.
- Security impact is considered.
- Tests are added or a reason is documented.
- Module docs are updated when module behavior changes.
- Completion report records commands and results.
- Remaining risks are named.

## P0 Done

P0 is done when:

- Product docs exist under `docs/00-product/`.
- Architecture docs exist under `docs/10-architecture/`.
- Security docs exist under `docs/20-security/`.
- Quality docs exist under `docs/50-quality/`.
- AI agent docs exist under `docs/ai/`.
- Root `CLAUDE.md` and `AGENTS.md` exist and are concise.
- MVP skill templates exist for Claude and portable Agent Skills-compatible tools.
- MCP strategy exists and keeps MCP guidance-only for MVP.

## Future Implementation Done

Future implementation work must include:

- Files changed.
- Tests added or updated.
- Commands run.
- Results.
- Skipped checks.
- Risks.
- Docs updated.
