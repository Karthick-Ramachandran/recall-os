...
ADR creation is idempotent by slug. If `ADR-####-deterministic-cache-policy.md` already exists,
`adr create deterministic-cache-policy` targets that file and lets the write policy skip or
overwrite it. New ADR slugs receive the next number after the highest valid ADR file.
