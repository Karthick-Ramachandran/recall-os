# PRD: Recall OS Package Release

## Purpose

Recall OS needs to move from an integration-tested TypeScript project to a publish-ready CLI
package.

This milestone also completes the pre-public rename from SpecForge to Recall OS so the product name,
package name, CLI command, generated memory, docs, examples, and tests all describe one coherent
product.

## Goals

- Rename tracked product references to Recall OS.
- Use npm package name `recall-os`.
- Expose CLI binary `recall`.
- Use `.recall/config.json` as the repository config path.
- Add build, package validation, CI, release-candidate validation, README, examples, and MIT
  license.
- Add `recall preset list`.
...
- A user can install from the local package artifact and run `recall`.
- `recall init` works in an empty directory.
- `recall doctor` validates an initialized repository.
- `recall preset list` reports built-in presets.
