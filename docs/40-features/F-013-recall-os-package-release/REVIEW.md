# Review: Recall OS Package Release
...
- Passed: `.recall/config.json` is the only supported config path.
- Passed: `recall` is the packaged CLI binary.
...
- Remaining release risk: P10 prepares the package but does not publish it. npm name ownership for
  `recall-os` must be checked again before a real publish.
